...
idx= 0
temp2 = pd.read_pickle('ex_traffic_'+str(int(idx/12)+2019)+'_01.pkl')
temp2.출발영업소명
dp_city = []
dp_prov = []
ar_city = []
ar_prov = []
idx= 0
for i in temp2.출발영업소명:
    if idx%100 ==0:
        print(idx, i)
    temp_cityname='NaN'
    jdx = 0
    for j in df_province.지명:
        if (Series(i).str.contains(j).astype('int32').sum()==1) :
            temp_cityname = i
            break
        jdx+=1
    dp_city.append(temp_cityname)
    dp_prov.append(df_province.도명[jdx])
    idx+=1
...
pd.merge(df_province, temp2, left='지명',right='출발영업소명')
df_traffic.info()
df_province.info()
